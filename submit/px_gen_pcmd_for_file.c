...
/*   Updated: 2024/07/10 16:00:26 by tookuyam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_px_pipe_cmd	**px_gen_pcmd_for_file(int argc, char *argv[])
{
	...
	while (index < argc - 1)
	{
		pipe_cmds[index - 2] = _px_gen_pipe_cmd_for_heredoc(index, argc, argv);
...
t_px_pipe_cmd	*_px_gen_pipe_cmd_for_heredoc(int index, int argc, char *argv[])
{
	...
	if (index == 2)
		return (px_new_pcmd(argv[index - 1], NULL, argv[index], here_fd));
